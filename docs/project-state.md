...
нативный `aria-pressed` вместо дублирующих icon/text/selected modifier classes;
exact geometry, measured indicator, rapid retarget, focus, forced colors и
reduced motion не меняются. Compact table `ActionMenu` стал явным component
variant `triggerSize="compact"`, поэтому Course, Lesson, Schedule и Students
больше не повышают specificity отдельными context classes. `AppPageHeader`
удалил redundant heading wrapper и classes, кодировавшие наличие back/actions,
сохранив reserved backlink row и title/action alignment.

Несмонтированный floating System Assistant launcher/panel и его глобальный
stylesheet удалены. Живая proposal/action карточка выделена в
`src/components/communication/assistant-action-card.tsx`, а assistant styles
принадлежат единственной current persisted surface — Communication Center.
Подтверждённо неиспользуемые selectors, tokens, старые component exports и
dashboard wrapper удалены; marketing reduced-motion reset ограничен landing
...
канонический `16 px` Lucide glyph со stroke `2 px` и стандартным SVG
`vector-effect` без mobile non-scaling override;
...
profile header full-bleed светлым divider и использует строки `68 px`. Подписи
пунктов меню имеют `14 px / 400`, имя — `14 px / 600`, email — `12 px / 400`,
а fallback-инициалы в `48 px` avatar наследуют `.72rem` (`11.52 px` только при
стандартном root `16 px`). Burger и пять menu glyphs используют те же
`16 px / 2 px` и стандартный vector rendering. Pointer-open не
...
`[data-variant="text"]` group on narrow/coarse viewports can shrink inside its
parent (`max-width: 100%`, `min-width: 0`, `flex-shrink: 1`); each option uses
`min-width: 0` and `flex: 1 1 0`. The visible
`.product-segmented-control-option-label` stays on one line with ellipsis,
while the unabridged button text remains its full accessible name. Exact
`80 × 40 px` geometry remains exclusive to a two-cell icon-only projection.
...
имеют независимый минимум `16 px` для защиты от iOS focus zoom. Pinch zoom при
этом не запрещается через viewport. Global launcher
...
в portal-menu; других кнопок действий в строке нет. Пункты
portal-menu имеют точную высоту 40 px, вертикально центрированные иконку и
текст, шрифт `.88rem/400`, icon `16 px` и канонические внутренние интервалы на
desktop, narrow и coarse.
...
- Один `WorkspaceTabs` используется во всех product consumers, включая Courses
  ...
  остаётся видимым поверх hover-фона. Tab имеет `16 px` Lucide-иконку со stroke
  `2 px` и стандартным vector rendering на desktop, narrow и coarse. Только
  ...
  fully opaque icons и единый `16 px / 2 px` icon rhythm; current-source
  narrow/coarse слой сохраняет эту desktop visual parity без отдельного
  type/icon override. Исходный
...
- В current production краткий Action «Назначить урок» с иконкой добавления в
  ...
  вертикально центрированы и используют `.88rem/400` на desktop, narrow и
  coarse. System Assistant намеренно
...
- System feed принимает только trusted typed application events. Первый
  ...
  произвольный `payload.href`. Learning Activity evidence/profile projections
  и background result producers не реализованы; sequencing находится в roadmap.
...
Identity/observer architecture и actor matrix находятся в
[`docs/architecture/learner-identity-access-model.md`](./architecture/learner-identity-access-model.md),
а rollout evidence — в deployment runbook. Production contract-stage завершён:
final exact functional web deploy, DB/API/HTTP и authenticated browser postflight
прошли 9 августа 2026 года.
