...
одной toolbar-строке с поиском и выбором вида и не переносится на страницу
открытого курса. Current source удаляет отдельную subject/level filter-кнопку;
поиск, audience direction и cursor продолжают работать. Backend-фильтрация и
facets остаются совместимым server-side contract внутри выбранного
направления, но active web UI больше не отправляет subject/level параметры.
