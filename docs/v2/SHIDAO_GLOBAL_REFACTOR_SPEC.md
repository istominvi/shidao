...
- Web: `https://shidao.ru`
- Ветка: `main`.
- На время реконструкции может показывать статическую страницу «ShiDao находится в разработке» либо текущее состояние V2.
...
Текущий сайт является средой разработки, а не рабочим production с пользовательскими данными. Поэтому отдельный V2-контур не является обязательным условием начала работ.
...
- использовать текущий web deployment и текущий Supabase stack;
...
26. Основной домен расположен на `shidao.ru`.
27. `staging.shidao.ru` является необязательным будущим окружением; если оно создаётся, то полностью изолируется.
