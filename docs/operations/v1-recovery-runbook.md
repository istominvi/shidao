...
9. Первая полная репетиция должна выполняться на одноразовом VDS. Активный V2 production-контур не является площадкой для первой проверки процедуры.
...
Ещё не подтверждено единственным окончательным способом: полный restore всего
набора на отдельном чистом VDS. Поскольку V2 production уже активен, отсутствие
этой репетиции остаётся явным recovery debt. Проверку нужно провести на
изолированном чистом VDS, не затрагивая active production-контур.
