...
9. Первая полная репетиция должна выполняться на одноразовом VDS. Активный тестовый контур не является площадкой для первой проверки процедуры.
...
Ещё не подтверждено единственным окончательным способом: полный restore всего набора на отдельном чистом VDS. Эта репетиция обязательна до превращения текущего тестового контура в production, но не блокирует разработку V2.
