...
короткие русские подписи периода и оставляет рядом только icon-only «Таблица /
Карточки». Белая таблица без внешней рамки получила однострочные отдельные
`Дата / Время / Урок / Курс / Ученики / Статус`, header ровно 40 px вместе с
разделителем и weight 500. Последняя колонка не имеет видимого заголовка:
...
сплошными белыми поверхностями без blur. Это UI-only polish без новой schema,
migration или API. Course workspace использует четыре вкладки;
...
русские даты, открывает календарь с «День / Неделя / Месяц» и стрелками
...
weight 500 и более светлый текст. Отдельные `Дата / Время / Урок / Курс /
Ученики / Статус` выводятся в одну строку с ellipsis, scheduled state остаётся
plain «Ожидается». В последней колонке постоянное вертикальное троеточие
...
поверхностями без blur. Schema/API не меняются; deployment этого polish
