...
import { ensureUserPreference, getUserContextById, setLastActiveProfile, upsertParentProfile, upsertTeacherProfile } from '@/lib/server/supabase-admin';
...
export async function POST(req: NextRequest) {
  ...
  if (!body.profile || !['parent', 'teacher'].includes(body.profile)) {
    return NextResponse.json({ error: 'Некорректный профиль.' }, { status: 400 });
  }

  const context = await getUserContextById(session.uid);
  if (context.actorKind === 'student') {
    return NextResponse.json({ error: 'Онбординг недоступен для ученика.' }, { status: 403 });
  }
