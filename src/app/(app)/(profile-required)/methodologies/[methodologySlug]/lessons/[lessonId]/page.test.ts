...
test("methodology lesson page uses shared teacher workspace tabs", () => {
  ...
  assert.equal(pageSource.includes("LessonMetaRail"), false);
  assert.equal(pageSource.includes("LessonMetaPill"), false);
  assert.equal(pageSource.includes("Source-урок методики"), false);
  assert.equal(pageSource.includes('eyebrow="Урок методики"'), false);
  assert.equal(pageSource.includes('icon="duration"'), false);
  assert.equal(pageSource.includes('tone="success"'), false);
