...
function HomeworkThreadPreview({
    ...
    .flatMap((thread) =>
      thread.messages.filter(
        (message: GroupStudentMessage) =>
          message.scheduledLessonHomeworkAssignmentId === item.scheduledHomeworkAssignmentId,
      ),
    )
    .slice(-2);
  ...
  return (
    <div className="mt-3 rounded-xl border border-neutral-200 p-2.5">
      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-neutral-500">Обсуждение задания</p>
      {messages.length === 0 ? (
      ...
      <form action="/api/student/communication" method="POST" className="mt-2 space-y-2">
        ...
        />
        <button type="submit" className="rounded-xl border border-neutral-300 px-3 py-1.5 text-xs font-semibold">
          Отправить
        </button>
      </form>
    </div>
        ...
        <ul className="space-y-2">
          {lessons.map((lesson) => (
            <li key={lesson.scheduledLessonId} className="rounded-xl border border-neutral-200 p-3">
              ...
              <Link href={toScheduledLessonRoute(lesson.scheduledLessonId)} className="mt-2 inline-flex rounded-xl border border-neutral-300 px-3 py-1.5 text-xs font-semibold text-neutral-800">
                Открыть урок
              </Link>
            </li>
          ))}
...
function StudentHomeworkItem({
  ...
  return (
    <li className="rounded-2xl border border-neutral-200 p-3">
      <div className="flex flex-wrap items-center gap-2">
        <p className="font-semibold">{item.homeworkTitle}</p>
        <span className="rounded-full bg-neutral-100 px-2 py-0.5 text-xs font-semibold text-neutral-700">{kindBadge(item.kind)}</span>
      </div>
      ...
      {item.kind === "practice_text" ? (
        <form className="mt-2 space-y-2" action={`/api/student/homework/${item.studentHomeworkAssignmentId}/submit`} method="POST">
      ...
      {item.reviewNote ? <p className="mt-2 text-sm text-neutral-700">Комментарий: {item.reviewNote}</p> : null}
...
export function StudentHomeworkList({
  ...
  return (
    <DashboardSection title="Мои домашние задания">
      {homework.length === 0 ? (
        <DashboardEmptyState>Пока преподаватель не выдал домашнее задание.</DashboardEmptyState>
      ) : (
        <ul className="space-y-3">
          {homework.map((item) => (
            <StudentHomeworkItem key={item.studentHomeworkAssignmentId} item={item} communication={communication} />
          ))}
        </ul>
      )}
    </DashboardSection>
