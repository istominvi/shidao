...
import { canRenderSessionNavActions, resolveLandingAuthCtaHref, resolveLandingNavAction, resolveTopNavAction, shouldRedirectSecuritySettingsToLogin } from '../navigation-contract';
import { ROUTES } from '../../lib/auth';
import type { SessionAdultView, SessionDegradedView, SessionGuestView, SessionStudentView } from '../../lib/session-view';

const guest: SessionGuestView = { kind: 'guest', authenticated: false };
const degraded: SessionDegradedView = { kind: 'degraded', authenticated: true, reason: 'context_unavailable' };
const adult: SessionAdultView = { kind: 'adult', authenticated: true, hasPin: false, activeProfile: null, availableProfiles: [] };
const student: SessionStudentView = { kind: 'student', authenticated: true, hasPin: true };
...
test('TopNav: protected route keeps guest CTA hidden', () => {
  assert.equal(resolveTopNavAction(ROUTES.settingsSecurity, guest, true), 'skeleton');
  assert.equal(resolveTopNavAction(ROUTES.dashboard, degraded, true), 'skeleton');
});

test('TopNav: authenticated session always renders session actions', () => {
  assert.equal(resolveTopNavAction(ROUTES.login, adult, true), 'session-actions');
  assert.equal(resolveTopNavAction(ROUTES.home, student, false), 'session-actions');
});
