...
test("world-around-me lesson 1 unified read model keeps canonical 16-step mapping", () => {
  ...
  assert.equal(step1.id, "canonical-world-around-me-lesson-1-step-1");
  ...
  assert.equal(step2.id, "canonical-world-around-me-lesson-1-step-2");
  ...
  assert.equal(step9.title, "Учим глаголы 跑，跳");
  ...
  assert.equal(step15.title, "Поём песню «farm animals»");
  ...
  assert.equal(step15.resourceIds?.includes("song-video:farm-animals-movement"), true);

  const step16 = unified.steps[15];
  assert.equal(step16.title, "Прощаемся с детьми и героями курса");
  ...
  assert.equal(totalMinutes >= 46 && totalMinutes <= 50, true);
