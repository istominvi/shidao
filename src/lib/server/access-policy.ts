...
export type AccessResolution =
  ...
  | { status: 'adult-with-profile'; context: UserContext; activeProfile: ProfileKind }
...
async function normalizeAdultContext(context: UserContext) {
  try {
    ...
    console.error('[access-policy] ensureUserPreference failed', { userId: context.userId, error });
  }
  ...
  if (context.availableAdultProfiles.length === 2 && !context.preferences?.last_active_profile) {
    try {
      ...
      console.error('[access-policy] setLastActiveProfile failed', { userId: context.userId, error });
    }
  }
}
...
export async function resolveAccessPolicy(): Promise<AccessResolution> {
  ...
  try {
    ...
    if (!activeProfile) {
      return { status: 'degraded', reason: 'adult profile exists but active profile is unavailable' };
    }

    return { status: 'adult-with-profile', context, activeProfile };
  } catch (error) {
    const reason = error instanceof Error ? error.message : 'Unknown access resolution error';
