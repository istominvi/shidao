...
export const GUEST_SESSION_VIEW: SessionGuestView = { kind: 'guest', authenticated: false };
...
function pickIdentity(input: Record<string, unknown>): SessionIdentity {
  return {
    ...
    fullName: typeof input.fullName === 'string' || input.fullName === null ? input.fullName : undefined,
    email: typeof input.email === 'string' || input.email === null ? input.email : undefined,
...
export function toSessionView(input: unknown): SessionView {
  ...
  switch (input.kind) {
    case 'guest':
      return GUEST_SESSION_VIEW;
    case 'student':
      if (input.authenticated !== true || typeof input.hasPin !== 'boolean') return GUEST_SESSION_VIEW;
      return {
        kind: 'student',
        authenticated: true,
        hasPin: input.hasPin,
    ...
    case 'adult':
      if (input.authenticated !== true || typeof input.hasPin !== 'boolean' || !Array.isArray(input.availableProfiles)) {
        return GUEST_SESSION_VIEW;
      }
      return {
        kind: 'adult',
        authenticated: true,
        hasPin: input.hasPin,
        activeProfile: isProfileKind(input.activeProfile) ? input.activeProfile : null,
